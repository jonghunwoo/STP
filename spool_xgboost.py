# 해당 파일은 앙상블학습의 xg boost 알고리즘을 적용한 학습모델을 구축하여 성능 확인을 수행 (xgboost 라이브러리 폴더가 작업 폴더내에 존재해야 실행 가능함)
# Package Load
import pandas as pd
import numpy as np
...
########################################### MakingLT ###################################################################
# Data 불러오기
XG_MakingLT = pd.read_csv('C:/Users/JJH/Desktop/JJH_KMOU/Study/2. Python/spool_pycharm/p_makingdata.csv',engine = 'python')
...
XG_m_predicted[XG_m_predicted<0] = 0
...
########################################### PaintingLT #################################################################
# Data 불러오기
XG_PaintingLT = pd.read_csv('C:/Users/JJH/Desktop/JJH_KMOU/Study/2. Python/spool_pycharm/p_paintingdata.csv',engine = 'python')

# 첫번째 Column 삭제
del XG_PaintingLT['Unnamed: 0']

# Data Type 변경
XG_PaintingLT["Emergency"] = XG_PaintingLT["Emergency"].astype(np.object)
...
XG_p_predicted[XG_p_predicted<0] = 0
